...
Client.prototype.createAddStream = function(){
   var path = [this.options.path,this.options.core,'update?commitWithin=6000&wt=json']
      .filter(function(element){
         if(element) return true;
         return false;
      })
...
Client.prototype.update = function(data,callback){
   ...
   this.options.fullPath = [this.options.path,this.options.core,'update?commitWithin=6000&wt=json']
                              .filter(function(element){
                                 if(element) return true;
                                 return false;
                              })
