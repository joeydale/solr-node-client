...
Client.prototype.addRemoteResource = function(options,callback){
   ...
   var path = 'update/' + options.format.toLowerCase()  + '?' + querystring.stringify(options.parameters) + '&wt=json&overwrite=' + self.overwrite;
...
Client.prototype.update = function(data,callback){
   ...
   this.options.fullPath = [this.options.path,this.options.core,'update/json?commit='+ this.autoCommit +'&wt=json&overwrite=' + self.overwrite]
                              .filter(function(element){
                                 if(element) return true;
                                 return false;    
                              })
