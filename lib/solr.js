 ...
 * Create an instance of `Client`
 *
 * @param {String} [url='http://localhost:8080/solr/mycore'] - The full path to your solr core
 *
 * @return {Client}
 * @api public
 */
exports.buildClient = function(url) {
   ...
   var options = {
      ...
      port: r.port || r.protocol === 'http:' ? 80 : 443,
      path: r.pathname
   };
   return new Client(options);
}
...
var Client = function(options){
   ...
   this.overwrite = false; // gain performance if we know we are not going to overwrite when indexing
}
...
Client.prototype.addRemoteResource = function(options,callback){
   ...
   var path = 'update/' + '?' + querystring.stringify(options.parameters) + '&wt=json&overwrite=' + self.overwrite;
...
Client.prototype.createAddStream = function(){
   var path = [this.options.path,this.options.core,'update?commit='+ this.autoCommit +'&wt=json&overwrite=' + self.overwrite]
      .filter(function(element){
         if(element) return true;
         return false;    
      })
...
Client.prototype.update = function(data,callback){
   ...
   this.options.fullPath = [this.options.path,this.options.core,'update?commit='+ this.autoCommit +'&wt=json&overwrite=' + self.overwrite]
                              .filter(function(element){
                                 if(element) return true;
                                 return false;    
                              })
